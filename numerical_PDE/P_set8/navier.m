function [U,V,P,X,Y] = navier(Re,nx,ny,dt,tf)
...
% March 2009 - minor modifications by Tina Katopodes Chow
%-----------------------------------------------------------------------
%Re = 1e2;     % Reynolds number INPUT ABOVE
%dt = 1e-2;    % time step INPUT ABOVE
%tf = 4e-0;    % final time INPUT ABOVE
lx = 1;       % width of box
ly = 1;       % height of box
%nx = 90;      % number of x-gridpoints INPUT ABOVE
%ny = 90;      % number of y-gridpoints INPUT ABOVEZZ
...
for k = 1:nt
   ...
   if k==1|floor(nsteps*k/nt)>floor(nsteps*(k-1)/nt)
      % calcualte stream function Q
      rhs = reshape(diff(U')'/hy-diff(V)/hx,[],1);
      q(perq) = Rq\(Rqt\rhs(perq));
      Q = zeros(nx+1,ny+1);
      Q(2:end-1,2:end-1) = reshape(q,nx-1,ny-1);
      clf, contourf(avg(x),avg(y),P',20,'w-'), hold on %contour pressure
      contour(x,y,Q',20,'k-'); %contour streamfunction
      Ue = [uS' avg([uW;U;uE]')' uN'];
      Ve = [vW;avg([vS' V vN']);vE];
      Len = sqrt(Ue.^2+Ve.^2+eps); %velocity magnitude
      quiver(x,y,(Ue./Len)',(Ve./Len)',.4,'k-') %add vectors
...
%TKC extracted as separate .m files
%function B = avg(A,k)
%if nargin<2, k = 1; end
